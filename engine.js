const GIPPP_ENGINE = (() => {
    ...
    const uiStrings = {
        ko: { desc: "글로벌 인사이트 프로파일러", security: "🔒 보안: 데이터 저장 안 함", processing: "분석 중...", wait: "잠시만 기다려 주세요.", saveImg: "📸 이미지 저장", retest: "다시 하기", reportTitle: "인사이트 리포트", recommendTitle: "💡 추천 상품", viewAmazon: "아마존 보기", traits: { E: "외향성", A: "친화성", C: "성실성", N: "신경증", O: "개방성" } },
        en: { desc: "Global Insight Profiler", security: "🔒 Security: No data stored", processing: "Analyzing...", wait: "Please wait...", saveImg: "📸 Save Image", retest: "Retest", reportTitle: "Insight Report", recommendTitle: "💡 Recommended", viewAmazon: "View on Amazon", traits: { E: "Extraversion", A: "Agreeableness", C: "Conscientiousness", N: "Neuroticism", O: "Openness" } }
    };
    const amazonProducts = { E: { keyword: "party games" }, A: { keyword: "gift sets" }, C: { keyword: "planner" }, N: { keyword: "meditation" }, O: { keyword: "art supplies" } };
    const ui = { brandDesc: document.getElementById('brand-desc'), securityNote: document.getElementById('security-note'), questionText: document.getElementById('question-text'), optionsGroup: document.getElementById('options-group'), progressFill: document.getElementById('progress-fill'), mainContent: document.getElementById('main-content') };
    ...
    const renderQuestion = () => {
        ...
        ui.questionText.innerHTML = `<div style="font-size:1rem; color:#3498db;">Q${state.currentIndex + 1}</div><div>${q.text}</div>`;
    ...
    const renderFinalReport = () => {
        ...
        for (const [trait, data] of Object.entries(state.results)) {
            ...
            reportHtml += `<div style="margin-bottom:20px;"><strong>${strings.traits[trait]} ${p}%</strong><div style="width:100%; height:10px; background:#eee; border-radius:5px; overflow:hidden;"><div style="width:${p}%; height:100%; background:#3498db;"></div></div><p style="font-size:0.9rem;">${p >= 50 ? state.descriptions[trait].high : state.descriptions[trait].low}</p></div>`;
        }
        reportHtml += `<div style="background:#fff9e6; padding:15px; border-radius:15px; text-align:center; margin-bottom:20px;"><h4>${strings.recommendTitle}</h4><a href="https://www.amazon.com/s?k=${amazonProducts[maxTrait].keyword}" target="_blank" style="color:#ff9900; font-weight:bold;">${strings.viewAmazon}</a></div>`;
        reportHtml += `<div style="text-align:center; margin-bottom:20px;"><img id="qrImage" src="${qrImgUrl}" crossorigin="anonymous" style="width:120px; border:5px solid white;"></div>`;
        reportHtml += `<button onclick="GIPPP_ENGINE.generateImage()" style="width:100%; padding:15px; background:#3498db; color:white; border:none; border-radius:10px; font-weight:bold; margin-bottom:10px;">${strings.saveImg}</button>`;
        reportHtml += `<button onclick="location.href=window.location.pathname" style="width:100%; padding:10px; background:#eee; border:none; border-radius:10px;">${strings.retest}</button></div><canvas id="resultCanvas" style="display:none;"></canvas>`;
    ...
    const generateImage = () => {
        ...
        ctx.fillStyle = 'white'; ctx.font = 'bold 30px sans-serif'; ctx.textAlign = 'center'; ctx.fillText('Insight Report', 300, 50);
        let y = 150;
        Object.entries(state.results).forEach(([t, d]) => {
            const p = d.count === 20 ? d.total : Math.round((d.total / (d.count * 5)) * 100);
            ctx.fillStyle = '#2c3e50'; ctx.font = '20px sans-serif'; ctx.textAlign = 'left'; ctx.fillText(t, 50, y);
            ctx.fillStyle = '#eee'; ctx.fillRect(150, y - 15, 400, 20);
            ctx.fillStyle = '#3498db'; ctx.fillRect(150, y - 15, 4 * p, 20);
            y += 60;
        });
        if (qrImg.complete) ctx.drawImage(qrImg, 225, 600, 150, 150);
