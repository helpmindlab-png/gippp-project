/**
 * [GIPPP] Global Insight Profiler Project - Core Engine v2.9
 * Fixes: Fixed UI Height, Premium Image Layout (1.png style), Multi-lang Sync
 */
...
const GIPPP_ENGINE = (() => {
    ...
    const uiStrings = {
        ko: { desc: "글로벌 인사이트 프로파일러", security: "🔒 보안: 데이터 저장 안 함", processing: "분석 중...", wait: "잠시만 기다려 주세요.", saveImg: "📸 이미지 저장", retest: "다시 하기", reportTitle: "인사이트 리포트", recommendTitle: "💡 맞춤 추천", viewAmazon: "아마존 보기", qrNote: "QR코드를 스캔하여 테스트 시작", traits: { E: "외향성", A: "친화성", C: "성실성", N: "신경증", O: "개방성" }, labels: ["전혀 아니다", "아니다", "보통이다", "그렇다", "매우 그렇다"] },
        en: { desc: "Global Insight Profiler", security: "🔒 Security: No data stored", processing: "Analyzing...", wait: "Please wait...", saveImg: "📸 Save Image", retest: "Retest", reportTitle: "Insight Report", recommendTitle: "💡 Recommended", viewAmazon: "View on Amazon", qrNote: "Scan QR to start test", traits: { E: "Extraversion", A: "Agreeableness", C: "Conscientiousness", N: "Neuroticism", O: "Openness" }, labels: ["Strongly Disagree", "Disagree", "Neutral", "Agree", "Strongly Agree"] },
        es: { desc: "Perfilador de Perspectiva Global", security: "🔒 Seguridad: Sin datos guardados", processing: "Analizando...", wait: "Por favor espere...", saveImg: "📸 Guardar Imagen", retest: "Reiniciar", reportTitle: "Informe de Perspectiva", recommendTitle: "💡 Recomendado", viewAmazon: "Ver en Amazon", qrNote: "Escanea para comenzar", traits: { E: "Extraversión", A: "Amabilidad", C: "Responsabilidad", N: "Neuroticismo", O: "Apertura" }, labels: ["Muy en desacuerdo", "En desacuerdo", "Neutral", "De acuerdo", "Muy de acuerdo"] },
        jp: { desc: "グローバル・インサイト・プロファイラー", security: "🔒 セキュリティ: データ保存なし", processing: "分析中...", wait: "少々お待ちください...", saveImg: "📸 画像を保存", retest: "再テスト", reportTitle: "インサイトレポート", recommendTitle: "💡 おすすめ商品", viewAmazon: "Amazonで見る", qrNote: "QRコードをスキャンして開始", traits: { E: "外向性", A: "協調性", C: "誠実性", N: "神経症傾向", O: "開放性" }, labels: ["全くそう思わない", "そう思わない", "どちらともいえない", "そう思う", "強くそう思う"] },
    ...
    const renderFinalReport = () => {
        ...
        reportHtml += `<div style="background:#fff9e6; padding:20px; border-radius:20px; text-align:center; margin:30px 0; border:1px solid #ffeaa7;"><h4>${s.recommendTitle}</h4><a href="https://www.amazon.com/s?k=${amazonProducts[maxTrait]}" target="_blank" style="color:#ff9900; font-weight:bold; text-decoration:none; font-size:1.1rem;">${s.viewAmazon}</a></div>`;
    ...
    const generateImage = () => {
        ...
        // 1.png 스타일의 깔끔한 레이아웃 설정
        canvas.width = 600; canvas.height = 900;
        ctx.fillStyle = 'white'; ctx.fillRect(0, 0, 600, 900);
        ...
        // 하단 QR 코드 (중앙 배치, 1.png 스타일)
        if (qrImg && qrImg.complete) {
            const qrSize = 180;
            ctx.drawImage(qrImg, (600 - qrSize) / 2, 650, qrSize, qrSize);
        }
